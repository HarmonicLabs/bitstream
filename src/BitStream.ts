...
import { getNOfUsedBits, getNLastBits } from "./utils/stuff";
...
export class BitStream
{
    ...
    get length(): number
    {
        ...
        return this._nInitialZeroes + getNOfUsedBits( this._bits );
    }
    ...
    constructor( bytes: bigint | Uint8Array | undefined , nInitialZeroes: number = 0 )
    {
        ...
        assert(
            isUint8Array(bytes),
            "expected a Uint8Array instance", {
                got: bytes,
        ...
        this._nInitialZeroes = (8 * allZeroesBytes) + (8 - getNOfUsedBits( BigInt( firstNonZeroByte ) ) );
    ...
    static fromBinStr( binStr : string ): BitStream
    {
        ...
        const firstOneAt = binStr.indexOf('1');
        ...
        return new BitStream(
            BigInt( `0b${binStr.slice(firstOneAt)}`),
            firstOneAt
        )
    }
    ...
    }
    {
        ...
        const effectiveInByteInitialZeroes: InByteOffset = ( 8 - getNOfUsedBits( BigInt( firstNonZeroByte ?? 0 ) ) ) as InByteOffset ;
    ...
    {
        const lostBits = getNLastBits( this._bits, byOffset );
...
export class BitStreamIterator
{
    ...
    {
        if( this._isDone() ) return { done: true, value: 0 };
